...
public class ControllerFilter extends AbstractController implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		if (srequest.getSession().getAttribute(User.ID) != null) {
			EWeblibThreadLocal.set(User.ID, srequest.getSession().getAttribute(User.ID));
		}

		DataBaseQueryBuilder loginExistsBuilder = new DataBaseQueryBuilder(User.TABLE_NAME);
		loginExistsBuilder.and(User.ID, EWeblibThreadLocal.getCurrentUserId());
		...
		if (!EweblibUtil.isEmpty(EWeblibThreadLocal.getCurrentUserId()) && !queryDao.exists(loginExistsBuilder)) {
